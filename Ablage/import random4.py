...
class Character:
    def __init__(self, name, age, breed, role, is_enemy=False):
        ...
        self.health = 100
        self.level = 1
        self.experience_points = 0  # Neues Attribut für EP
        self.inventory = []
    ...
    def add_to_inventory(self, item, quantity=1):
        ...
        for item_stack in self.inventory:
            if item_stack["item"] == item:
                # Wenn das Element bereits vorhanden ist, erhöhen Sie die Menge
    ...
    def travel_and_encounter(self, destination_name, locations):
        ...
        if destination:
            if self.level >= 100:  # Überprüfung des Charakter-Levels
                self.current_location = destination
                print(f"{self.name} ist nach {destination.name} gereist.")
...
spider = Monster("Spinne")
wild_boar = Monster("Wildschwein")
wolf = Monster("Wolf")
troll = Monster("Troll")
monster_list = [spider, wild_boar, wolf, troll]

dungeon = Dungeon("Waldverlies", "Ein gefährliches Dungeon im Finsterwald")

dungeon.add_random_monster("Spinne", 1, 19)
dungeon.add_random_monster("Wildschwein", 20, 49)
dungeon.add_random_monster("Wolf", 50, 79)
dungeon.add_random_monster("Troll", 80, 89)
...
# Ändern Sie die Überprüfung der Benutzereingabe nach dem Ortswechsel
